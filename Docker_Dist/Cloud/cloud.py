...
print(f"receiveMax start:{time.time()-tStart}")
addrs,mTot= receiveMinMax(ipCloud, portC,numDevice)
portC += 1
print(f"receiveMax end:{time.time()-tStart}")
print(mTot)

#send to edge
print(f"sendMinMaxToEdge start:{time.time()-tStart}")
sendMinMaxToEdge( addrs, mTot, portE)
portE += 1
print(f"sendMinMaxToEdge end:{time.time()-tStart}")
print("[+] response sended")

#unione dei dati aggregati forniti dai dispositivi edge
print(f"receiveDataAgg start:{time.time()-tStart}")
dataAgg = receiveDataAgg(ipCloud, portC, numDevice)
portC += 1
print(f"receiveDataAgg end:{time.time()-tStart}")
...
sendClusterDict(clusterDict, addrs, portE)
